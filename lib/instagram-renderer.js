...
function wrapText(text, maxCharsPerLine, maxLines) {
  ...
  if (lines.length === maxLines && lines.join(" ").length < src.length) {
...
export async function renderInstagramCard({ category, title, summary, bullets = [], interpretation }) {
  ...
  const interpretationLines = wrapText(
    interpretation || "오늘 흐름의 방향성과 다음 확인 포인트를 함께 보세요.",
    38,
    2
  );
      ...
      children: [
        ...
        },
        {
          ...
          props: {
            ...
            children: interpretationLines.map((line, index) => ({
              type: "div",
              props: { key: `i${index}`, style: { flexShrink: 0 }, children: line }
            }))
          }
        },
        { type: "div", props: { style: { flex: 1, minHeight: "10px" } } },
        {
          type: "div",
          props: {
            style: {
